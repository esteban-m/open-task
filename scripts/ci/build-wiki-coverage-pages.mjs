...
function parseArgs(argv) {
  ...
  if (opts.packages.length === 0) {
    console.error(
      'Usage: build-wiki-coverage-pages.mjs --out-dir wiki-out --sha SHA --run-url URL '
...
function relSourcePath(fileKey, repoRoot) {
...
function detailTable(summaryPath, repoRoot) {
...
for (const pkg of opts.packages) {
  ...
  const content = isIndex
    ? buildIndexPage(opts.packages, meta)
    : buildPackagePage(pkg, meta);
